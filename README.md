...
- **Client:** Next.js (App Router) + React, TailwindCSS, shadcn/ui + Radix UI (Popover, Dialog, ScrollArea, DropdownMenu, Toast), Zustand for client state, zod for validation, **Three.js** for rendering (greybox primitives first, swap with real assets later). Optional: framer-motion, clsx, tailwind-merge, react-use.
...
/packages
  ...
  /rendering     # Three.js scene + rendering bridge (no game logic)
...
- `/apps/web`: Next.js routes for marketing/play/lobby/match, React overlay UI (shadcn/radix), Zustand stores, net client, and Three.js mount/bridge.
...
- `/packages/rendering`: Three.js scene + BeatSequencer-style sequencing + VfxDirector-style helpers consuming render commands/asset resolvers (no gameplay decisions); use greybox primitives until authored models exist.
...
- **W6 Rendering:** Three.js scene with greybox primitives (track, cars, simple VFX) consuming slot summaries. Outcome: 4-beat animations per turn via render commands.
...
6. **Build client overlay** (`apps/web`) with PlayLine UI, hand display, HUD, networking wrapper, and a Three.js bridge stub.
7. **Establish asset pipeline** (`packages/content` + `/assets`): placeholder manifest, `ASSET_GUIDE.md`, and audit script.
8. **Replace Three.js stub** with a track scene (greybox primitives first) plus BeatSequencer-style timing and VfxDirector helpers consuming render commands and manifest-driven assets.
...
- **React ↔ Three.js:** React converts `TurnSummary` into typed render commands (TurnStart → SlotResolve x4 → TurnEnd) consumed by a beat sequencer driving the Three.js scene.
